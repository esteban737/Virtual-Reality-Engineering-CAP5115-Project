	...
	public class XRRayInteractor : XRBaseControllerInteractor, IUIInteractable
    {
        ...
        public override void GetValidTargets(List<XRBaseInteractable> validTargets)
        {
            ...
            if (m_HitCount > 0)
			{
                ...
                for (var i = 0; i < Math.Min(m_HitCount, kMaxRaycastHits); ++i)  
			    {
			        ...
			        if (interactable && (m_RaycastHits[i].normal.y >= .7) && !validTargets.Contains(interactable))
        ...
        private void Update()
        {
            ...
            if (controller != null && controller.enableInputActions)
            {
                ...
                Vector2 axis;
                if (device.TryGetFeatureValue(axisFeature, out axis))
                {
                    if (axis.y < 0)
                    {
                        if (endPointDistance > -20)
                        {
                            ...
                            if (endPointDistance < 30)
                            {
                                endPointHeight -= .05f;
                                controlPointHeight -= .01f;
                            }
                        }
                    }
                    else if (axis.y > 0)
                    {
                        if (endPointDistance < 60)
                        {
                            endPointDistance += .2f;
